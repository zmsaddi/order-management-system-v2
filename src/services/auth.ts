...
export const useAuth = () => {
   ...
   * Get user profile with retry mechanism
   */
  private async getUserProfileWithRetry(userId: string, maxRetries: number = 3): Promise<AuthUser | null> {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.error(`Get user profile attempt ${attempt} failed:`, error)
   ...
   */
  private async logAuthEvent(event: string, userId?: string, success: boolean = true, errorMessage?: string): Promise<void> {
    try {
      const logData: any = {
        table_name: 'auth_events',
        record_id: userId || 'unknown',
      ...
      if (!success && errorMessage) {
        logData.new_values.error = errorMessage
      }
  ...
  private getErrorMessage(error: AuthError): string {
    const errorMap: Record<string, string> = {
      ...
      'Email not confirmed': 'يرجى تأكيد عنوان البريد الإلكتروني',
      'Too many requests': 'محاولات تسجيل دخول كثيرة جداً. يرجى المحاولة لاحقاً',
      'User not found': 'لا يوجد حساب بهذا البريد الإلكتروني',
      'Account locked': 'الحساب مقفل مؤقتاً بسبب محاولات تسجيل دخول متعددة',
    ...
    if (error.message.includes('500') || error.message.includes('502') || error.message.includes('503')) {
      return errorMap['Server error']
    }

    return errorMap[error.message] || error.message || 'حدث خطأ غير متوقع'
  }
